...
function doesWordExist(array, searchWord) {
  for (let i=0; i<array.length; i++){
    if (array[i] != array.includes(searchWord)){
      return false;
    } 
  }
...
function howManyTimes(objects, repWords) {
  let counter = 0;
  if (objects.length === 0) return counter;
...
for (let i=0;i<wordsCount.length; i++) {
  counter_rep.push(howManyTimes(wordsCount, wordsCount[i]));
  console.log(i);
  console.log(wordsCount[i]);
  console.log(counter_rep[i]);
}
